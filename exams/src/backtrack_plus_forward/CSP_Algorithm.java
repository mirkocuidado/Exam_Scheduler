package backtrack_plus_forward;
					...
					if(e.getNumOfPlaceTaken() <= e.getNumberOfSignedStudents()) {
						...
						for(int j=0; j<copy_termins.size(); ++j) {
							...
							/*** IF THE CURRENT EXAM AND THE CHOSEN ONE ARE THE SAME***/
								...
								/*if(!(termin.getDay() == copy_termins.get(j).getDay() && !termin.getRoom().equals(copy_termins.get(j).getRoom()) && termin.getTime().equals(copy_termins.get(j).getTime()))) {
									toBeRemoved.add(copy_termins.get(j));
								}
								if(termin.getDay() == copy_termins.get(j).getDay() && termin.getTime().equals(copy_termins.get(j).getTime())) {
									for(Exam exam_remover: exams) {
										if(exam.equals(exam_remover)) {
											exam_remover.removeTerminFromList(copy_termins.get(j));
											break;
										}
									}
								}*/
